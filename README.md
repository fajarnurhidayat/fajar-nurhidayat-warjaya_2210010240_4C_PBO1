...
1. **Class** adalah template atau blueprint dari object. Pada kode ini, `pemainbola`, `bolajaya`, dan `peainboladata` adalah contoh dari class.
...
2. **Object** adalah instance dari class. Pada kode ini, `pbl[i] = new pemainboladata(nama, id);` adalah contoh pembuatan object.

```bash
mhs[i] = new pemainboladata(nama, id);
```
...
4. **Constructor** adalah method yang pertama kali dijalankan pada saat pembuatan object. Pada kode ini, constructor ada di dalam class `pemainbola` dan `pemainboladata`.
...
8. **Inheritance** adalah konsep di mana sebuah class bisa mewarisi property dan method dari class lain. Pada kode ini, `pemainboladata' mewarisi `pemainbola` dengan sintaks `extends`.
